...
!  Check if $define card.
   39    if (ihld(1)(2:8) .ne. 'define') go to 46
...
!  Delete any exhausted if's from table.
  ...
  190  if (.not.chng) go to 200
       ...
       enddo
!  Write card.
  200  if (.not.skpprnt) write (out,'(a)') buf
       skpprnt = .false.
