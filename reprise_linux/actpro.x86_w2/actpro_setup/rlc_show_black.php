...
/*****************************************************************************
	...
	authorization from Reprise Software Inc.
 	...
 	$Id: rlc_show_black.php,v 1.8 2016/04/08 02:55:46 matt Exp $
...
include 'login_session.php';
	...
	{
	    $get = rlc_web_get_pagination($r1, $rpp, $tr, $sc, $sd, 
								"rlc-rpp-blk");

	    rlc_web_info("These domains will not be able to perform any activation functions on your activation server.<br><br>Note: The blacklist code does not require an exact domain name match, the name on<br>the blacklist only need match a substring of the actual domain name.<br>Therefore, placing \"foo.bar\" on the blacklist will blacklist not only \"foo.bar\" but also \"xyz.foo.bar\", etc.");
	    ...
	    rlc_web_row_start();
	    $xtra = "rpp=".$rpp."&tr=".$tr;
	    rlc_web_header_elem("Blacklisted domains", $xtra, "domain", $sc, 
	    								$sd);
	    rlc_web_row_end();
	    $numdisp = 0;
 ...
 */
		    ...
		    {
			$extra = "<input type=hidden name=black_id value=".
				$row['id'].
