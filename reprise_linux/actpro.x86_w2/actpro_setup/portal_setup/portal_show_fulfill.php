...
/*****************************************************************************
	...
	authorization from Reprise Software Inc.
 	...
 	$Id: portal_show_fulfill.php,v 1.25 2016/10/14 17:39:34 matt Exp $
...
function get_selection_data($cols, $sel_data)
{
	$head = array("Product Name", "Activation Key", "License Hostid", 
			...
			"Logged Data");
	rlc_web_browse_select("portal_show_fulfill.php", $sel_data, $head, $cols);
}
	...
	$back = rlc_getinput('back', "portal_show_fulfill.php");

	if ($back == "portal_show_fulfill.php") $backtxt = "";
	else				     $backtxt = "Back";
 ...
 */
	...
	$stat = portal_mysql_get_fulfillments($session->company_id, $sql, $select, $fulfill, $sc, $sd);
	if ($stat != 0)
	{
	...
	rlc_web_paginate("portal_show_fulfill.php", $extra, $pextra, $rpp, 
						$r1, -1, $tr, $sc, $sd, "");
 ...
 */
		...
		rlc_show_button('portal_show_lic.php', $bxtra);
	        if ($perm['act_edit_enabled'])
	        {
		    $cxtra = $pextra.  
