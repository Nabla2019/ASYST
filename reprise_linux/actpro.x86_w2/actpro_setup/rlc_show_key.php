...
/*****************************************************************************
	...
	authorization from Reprise Software Inc.
 	...
 	$Id: rlc_show_key.php,v 1.35 2017/03/07 01:02:41 matt Exp $
...
function get_selection_data($cols, $sel_data)
{
	...
	rlc_web_browse_select("rlc_show_key.php", $sel_data, $head, $cols);
}
 ...
 */
	...
	/* P379 - changed "exp" to "keyd.exp" below */
	$cols = array("keyd.akey", "name", "keyd.exp", "kver", "white", 
				"keyd.misc", "notes", "contact", "email",
				"company");
 ...
 */
	...
	$stat = rlc_mysql_get_keys_with_contact($sql, $keys, $sc, $sd, $select);
 ...
 */
	...
	rlc_web_paginate("rlc_show_key.php", $xtra, $pxtra, $rpp, $r1, -1, 
							$tr, $sc, $sd, "");
	...
	rlc_web_header_elem("Activation Key", $sort, "keyd.akey", $sc, $sd);
 ...
 */
	...
	/* P379 - changed "exp" to "keyd.exp" below */
	rlc_web_header_elem("Exp", $sort, "keyd.exp", $sc, $sd);
 ...
 */
		...
		if ($num > 0)
		{
		    $extra = $pxtra.
			      ...
			      rlc_web_hidden_string("back", "rlc_show_key.php");
		    rlc_show_button("rlc_show_fulfill.php", $extra);
		}
/*
 ...
 */
	...
	rlc_web_paginate("rlc_show_key.php", $xtra, $pxtra, $rpp, $r1, 
