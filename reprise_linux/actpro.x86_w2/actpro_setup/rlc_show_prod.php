...
/*****************************************************************************
	...
	authorization from Reprise Software Inc.
 	...
 	$Id: rlc_show_prod.php,v 1.24 2017/05/02 01:11:46 matt Exp $
...
function get_selection_data($cols, $sel_data)
{
	...
	rlc_web_browse_select("rlc_show_prod.php", $sel_data, $head, $cols);
}

function clone_product($sql, $session, $prod_id, $product, $select)
{
	echo "Cloning product... to $product<br>";
	$stat = rlc_mysql_get_products($sql, $prods, "", "", $select);
 ...
 */
	...
	rlc_web_title($title, "rlc_show_prod.php");
 ...
 */
	echo "<form method='get' action='rlc_show_prod.php'></form>";
 ...
 */
	...
	$stat = rlc_mysql_get_products($sql, $prods, $sc, $sd, $select);
 ...
 */
	...
	rlc_web_paginate("rlc_show_prod.php", $xtra, $pxtra, $rpp, $r1, -1, 
							$tr, $sc, $sd, "");
 ...
 */
		    $stat = rlc_mysql_get_products($sql, $thisprod, "", "", 
 ...
 */
		...
		else
		{
		    ...
		    $pextra = $pxtra.
			    ...
			    rlc_web_hidden_string("view_name", $row['name']);
		    if ($perm['act_edit_enabled'])
			edit_button("rlc_show_prod.php", 
						make_prod_extra($row, $pextra));
		    else if ($multi)
			rlc_show_button("rlc_show_prod.php", 
						make_prod_extra($row, $pextra));
		}
/*
 ...
 */
	...
	rlc_web_paginate("rlc_show_prod.php", $xtra, $pxtra, $rpp, $r1, 
							$r1 + $numdisp - 1,
						      $tr, $sc, $sd, "");
	if ($oneproduct)
	{
	    ...
	    if ($perm['act_edit_enabled'])
	    {
			...
			"View Complete Product list", "rlc_show_prod.php",
			"Add New License to this product", 
			"rlc_create_prod.php", "", $xtra);
		else finish_page_extra(0, 
			"Copy (clone) this product", "rlc_show_prod.php", 
			"View Complete Product list", "rlc_show_prod.php",
			"Add New License to this product", 
